/**
 * WebWorker que procesa la imagen: blanco y negro, detección de las esquinas
 * del DNI (por máscara de contraste y por rectas dominantes), refinado,
 * validación, enderezado por perspectiva y giros.
 * Conserva la última foto en memoria para re-procesarla (giros, re-detección)
 * sin volver a recibirla.
 */
import type { Punto, Esquinas, Rectangulo, PeticionWorker, ConId } from './tipos';
...
function ConvertirBN(imgPixels: ImageData): void {
...
function PonerHorizontal(img: ImageBitmap): OffscreenCanvas {
...
function ReducirAnchura(canvas: OffscreenCanvas): OffscreenCanvas {
 ...
 */
function AclararNegros(imgPixels: ImageData): ImageData {
...
function Mediana(muestras: number[]): number {
...
function OrdenarEsquinas(puntos: Punto[]): Esquinas {
...
function AreaCuadrilatero(esquinas: Punto[]): number {
	let area = 0;
	for (let i = 0; i < 4; i++) {
		const a = esquinas[i];
...
function GirarImageData(origen: ImageData, girar: number): ImageData {
...
function BitmapDeImageData(imgPixels: ImageData): ImageBitmap {
 ...
 */
function DetectarTarjeta(imgPixels: ImageData, forzar?: boolean): Deteccion | null {
 ...
 */
function DetectarPorLineas(imgPixels: ImageData, areaMinima: number): Deteccion | null {
	const w = imgPixels.width;
	const h = imgPixels.height;
	const data = imgPixels.data;
	...
	let mejor: { cuadrilatero: Esquinas; puntuacion: number } | null = null;
...
function PuntuarCuadrilatero(imgPixels: ImageData, esquinas: Punto[]): number {
	...
	function gris(x: number, y: number): number {
 ...
 */
function TonoInterior(imgPixels: ImageData, esquinas: Punto[]): number {
	const data = imgPixels.data;
 ...
 */
function AfinarYValidar(imgPixels: ImageData, aproximadas: Punto[], barridoAmplio = true): Deteccion | null {
 ...
 */
function EnderezarTarjeta(imgPixels: ImageData, esquinas: Esquinas): { imgPixels: ImageData; tarjeta: Rectangulo } | null {
	const w = imgPixels.width;
	const h = imgPixels.height;
	const [tl, tr, br, bl] = esquinas;
